...
var GetSellMiners = function (lRange) {
    ...
    if(OnSellMiners!=null){
        ...
        var sortedSellIndexList = OnSellMiners;

        for(var key in sortedSellIndexList){
            if(sortedSellIndexList[key].level <= lRange){
                continue;
            }
            minerList[key] = sortedSellIndexList[key];
            result = CreateSingleMinerSell(key,sortedSellIndexList[key]['mname'],sortedSellIndexList[key]['mprice'],sortedSellIndexList[key]['daylimt']) + result;
        }
...
var CreateSingleMinerSell = function (type,name,price,daylimt) {
    return '<a id="'+type+'" href="#" class="weui-media-box weui-media-box_appmsg" >'+
        ...
        '<img class="weui-media-box__thumb" src="http://fit.antit.top/miner/'+type+'" alt="">'+
        '</div>'+
        '<div class="weui-media-box__bd">'+
        '<h4 class="weui-media-box__title tw text-center"><strong class="">'+name+' ￥'+price/100+'/台</strong></h4>'+
    '<p class="tw">1年内，每日运动目标（1万步），当日最多可挖'+daylimt+'枚FIT；连续100天达标，奖1万枚FIT！</p>'+
    '</div>'+
    '</a>';
}
